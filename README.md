...
#### 2. 非侵入性
遵循面向切面开发思想，在小程序原有功能基础上进行扩展，所有功能的实现无任何破坏性，无论是引入使用方式和工具选项的传参方式都极具兼容性，不使用时去掉引用即可，代码无需任何更改，小程序照跑不误。
...
执行 setData 时工具内部会对比旧数据（`this.data`）和新数据，上面的写法在 setData 之前旧数据就已经变成新数据了，即旧数据是 `{obj: {a: 6}}`，新数据也是 `{obj: {a: 6}}`，那 diff 的结果就是没有发现变化，从而不触发 setData，显然工具如果直接忽略这次的 setData 那就会造成大 Bug，所以遇到这种情况内部会直接调用原生 setData。

所以当遇到更新字段为一个可变的引用类型时，工具会直接使用原生 setData 以确保你的代码可以正常运行，只是工具实际上就发挥不了任何优化作用了，所以相当于已经判断完了结果说这个优化不了还是用原生吧，那还不如刚开始就用原生了呢，多此一举...


那有没有办法解决呢？ 有！那为啥不解决？因为不合算，思路是再维护一份与渲染层数据完全一致的数据用于新旧数据的判断，该数据只受 `setData` 影响，不会被 `this.data` 的操作影响，理论上绝对可行，但是处于性能成本考虑这么做的必要性不大。

不随意操作 `this.data` 以及其指向的引用对象是一个对大家都好的习惯。
...
> 推荐回过头来读一下优化 setData 的来龙去脉，[小程序极致性能优化之 setData，让你的 setData速度翻倍提升](https://juejin.cn/post/7155646965978497032)